...
class CLIWatchAdapter implements IStorageEventWatchAdapter {
    ...
    private _relativeVaultPath(filePath: string): FilePath {
        return path.relative(this.basePath, filePath).replace(/\\/g, "/") as FilePath;
    }

    private _toNodeFile(filePath: string, stats: Stats | undefined): NodeFile {
        return {
            path: this._relativeVaultPath(filePath),
            stat: {
    ...
    private _toNodeFolder(dirPath: string): NodeFolder {
        return {
            path: this._relativeVaultPath(dirPath),
            isFolder: true,
        };
    }
    ...
    async beginWatch(handlers: IStorageEventWatchHandlers): Promise<void> {
        ...
        const ignored = rules
            ? [...baseIgnored, (p: string) => rules.shouldIgnore(this._relativeVaultPath(p))]
            : baseIgnored;
